...
import { createGeocodingJpClient } from "./geocoding-jp-client";
import fetch from "./gm-fetch";
import {
    AsyncOptions,
...
async function asyncMain({ L, map }: ContentGlobal) {
...
`;
    ...
    const geocoding = createGeocodingJpClient();
    async function suggestLocations(
        ...
        if (coordinate) {
            suggestions.push({ type: "coordinate", coordinate });
        }
        let result;
        try {
            result = await geocoding.search(
                { q: command },
                { ...options, fetch }
            );
        } catch (e) {
        ...
        suggestions.push({ type: "coordinate", coordinate: { lat, lng } });
    ...
    async function startNavigation(
        ...
        try {
            const result = await geocoding.search(
                { q: command },
                { ...options, fetch }
            );
    ...
    class Navigation extends L.Control {
        ...
        override onAdd(parentMap: L.Map) {
            ...
            const leftArrowButton = (
                <button>{materialSymbols.arrow_back}</button>
            );
